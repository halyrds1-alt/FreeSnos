...
def register_handlers(bot_instance, is_main=False, mirror_token=None):
    ...
    def text_handler(message):
        ...
        if state == 'admin_broadcast' and uid == ADMIN_ID:
            success, failed = broadcast_to_all_users(text, exclude=[ADMIN_ID])
            bot_instance.send_message(uid, f"📢 Рассылка завершена!\n✅ Успешно: {success}\n❌ Неудачно: {failed}")
            del temp_data[uid]
            return
...
def handle_admin_callback(bot_instance, call):
    ...
    if data == "admin_broadcast":
        temp_data[uid] = {'state': 'admin_broadcast'}
        safe_edit_message(bot_instance, "Введите текст рассылки (будет отправлено через основного бота):", uid, call.message.id, reply_markup=cancel_kb())
        return
...
mirrors_from_db = db.all_mirrors()
...
# ========== ФУНКЦИЯ РАССЫЛКИ ВСЕМ ПОЛЬЗОВАТЕЛЯМ (ДЛЯ АДМИНА) ==========
def broadcast_to_all_users(text, exclude=None):
    exclude = exclude or []
    users = db.all_users()
    success = 0
    failed = 0
    for uid in users:
        if uid in exclude:
            continue
        try:
            main_bot.send_message(uid, text, parse_mode='HTML')
            success += 1
        except Exception as e:
